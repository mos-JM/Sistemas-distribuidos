...
int main(int argc, char** argv){
    ...
    if(rank == MASTER){
        ...
        numRow = imgInfoHeaderInput.biHeight / (numProc - 1);
        rowRemainder = (imgInfoHeaderInput.biHeight % (numProc - 1)) + numRow;
        
        printf("TamEnvio init %d\n", numRow);
        printf("Envio resto la final %d\n", rowRemainder);
        ...
        /*
        recibimos las filas procesadas,
        filas * rowsize actualizando totalRowRecived
        calculamos puntero Outputbuffer
        recibimos bufferWorker
        copiamos en output buffer
        */
        ptrOutputBuffer = outputBuffer;
        int indice;
        for(i = 1; i < numProc ; ++i){
            MPI_Recv(&rowRecived,1, MPI_INT, MPI_ANY_SOURCE, flag,MPI_COMM_WORLD, &status);
           printf("rowRecived = %d", rowRecived);
            ptrOutputBuffer = outputBuffer + (imgInfoHeaderInput.biHeight / (numProc - 1)) * rowSize * (status.MPI_SOURCE - 1);
